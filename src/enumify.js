...
export class Enum {
  ...
  static _enumValuesFromArray(arr) {
    for (let i = 0; i < arr.length; i++) {
      this._pushEnumValue(new this(), arr[i][0], arr[i][1]);
    }
  }
  ...
  /**
   * Given the name of an enum constant, return its display.
   */
  static enumOrdinalOf(ordinal) {
    return this.enumValues.find(x => x.ordinal === ordinal);
  }
